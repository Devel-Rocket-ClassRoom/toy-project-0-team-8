...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField]
    private Background _backgroundRendererA;

    [SerializeField]
    private Background _backgroundRendererB;
    ...
    [SerializeField]
    private TextMeshProUGUI _earnedCoinText;
    ...
    [SerializeField]
    private InGameGearSlot _firstGearSlot;

    [SerializeField]
    private InGameGearSlot _secondGearSlot;

    [SerializeField]
    private InGameGearSlot _thirdGearSlot;

    [Header("=== 착용한 유물을 각각 표현할 1, 2, 3번 이미지 ===")]
    [SerializeField]
    private Image _firstGearImage;

    [SerializeField]
    private Image _secondGearImage;

    [SerializeField]
    private Image _thirdGearImage;
    ...
    public void LoadNextStage()
    {
        ...
        _stageRoot.transform.position = new Vector3(
            0,
            _stageRoot.position.y,
            _stageRoot.position.z
        );
    }
    ...
    private IEnumerator CoGrowToGiant(bool beBigger)
    {
        ...
        Vector3 endScale = beBigger
            ? _initialCookieScale * _giantScaleMultiplier
            : _initialCookieScale;

        while (growTimer <= _giantGrowDuration)
        {
            growTimer += Time.deltaTime;
            _cookieController.transform.localScale = Vector3.Lerp(
                startScale,
                endScale,
                growTimer / _giantGrowDuration
            );
            yield return null;
        }
