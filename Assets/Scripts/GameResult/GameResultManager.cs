...
public class GameResultManager : MonoBehaviour
{
    ...
    private Sprite GetGradeSpriteByStageNum(int stage) =>
        stage switch
        {
    ...
    private string GetStageResultStringByStageNum(int stage) =>
        stage switch
        {
            1 => $"스테이지 1 : 마녀의 집",
            2 => $"스테이지 2 : 젤리벌레의 숲",
    ...
    private IEnumerator CoUpdateResult()
    {
        ...
        while (timer <= _lerpDuration)
        {
            ...
            _coinText.text = ((int)Mathf.Lerp(0, _earnedCoin, timer / _lerpDuration)).ToString(
                "N0"
            );
            yield return null;
        }
        ...
        // Best Score 갱신시에만 표시
        if (_isMaxScoreRenewed)
        {
            _bestScoreImage.color = new Color(1, 1, 1, 1);
        }
